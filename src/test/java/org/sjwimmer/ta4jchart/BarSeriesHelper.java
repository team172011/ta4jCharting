package org.sjwimmer.ta4jchart;
...
public class BarSeriesHelper {
    ...
    public static List<Bar> createBars(final double... closes){
        ...
        for(var i = 0; i < closes.length; i ++){
            bars.add(new BaseBar(Duration.ofDays(1), ZonedDateTime.now().minusDays((long) (closes.length - i)), closes[i], closes[i] + (double) i, Math.max((double) 0, closes[i] - i), closes[i], closes[i] * (double) i));
        }
        return bars;
    }
